...
$role = user()['role'];$stage = strtolower($role) === 'department officer' ? 'department' : 'fams';$allowedDecisions = ['approved', 'rejected', 'needs_correction'];
...
try {
    ...
    // Prepare clear, professional notification templates
    switch ($newStatus) {
        case 'under_review':
            $subject = "Application Under Review";
            $message = "Your application has been verified by the department and is currently undergoing final review.";
            break;

        case 'approved':
            $subject = "Application Approved";
            $message = "Congratulations! Your field application has been officially approved. Check your dashboard for placement details.";
            break;

        case 'returned_for_correction':
            $subject = "Action Required: Application Correction Needed";
            $message = "Your application requires updates before it can proceed. Please review the comments on your dashboard and resubmit.";
            break;

        case 'rejected':
            $subject = "Application Decision Update";
            $message = "Your application review is complete. Please log in to your dashboard to review the decision details.";
            break;

        default:
            $subject = "Application Status Updated";
            $message = "The status of your application has been updated to " . str_replace('_', ' ', $newStatus) . ".";
            break;
    }
    ...
    $history = $pdo->prepare('INSERT INTO application_status_history (application_id, previous_status, new_status, changed_by, comment) VALUES (?, ?, ?, ?, ?)');$history->execute([$applicationId,$current['status'], $newStatus, user()['id'],$comment ?: null]);
    ...
    $notice->execute([$current['student_user_id'],$applicationId, 'application_review', $subject,$message]);
